...
async def create_availability(payload: DisponibilidadCreate, db: AsyncSession = Depends(get_db)) -> DisponibilidadMedica:
    item = DisponibilidadMedica(**payload.model_dump())
    db.add(item)
    await db.commit()
...
async def update_availability(availability_id: int, payload: DisponibilidadUpdate, db: AsyncSession = Depends(get_db)) -> DisponibilidadMedica:
...
async def delete_availability(availability_id: int, db: AsyncSession = Depends(get_db)) -> dict[str, str]:
    item = await db.get(DisponibilidadMedica, availability_id)
    if item is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Availability not found")
